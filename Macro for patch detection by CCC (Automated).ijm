...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	dir2 = dir1+"/../OutputCCC"+File.separator; //this creates the new path; "/../"selects parent folder
	...
	//define neck line
	waitForUser("Draw Horizontal across neckline");
	getLine(x1, y1, x2, y2, lineWidth);
	Ys1 = y1;
	
	//define pelvis level
	waitForUser("Draw Horizontal across pelvis");
	getLine(x1, y1, x2, y2, lineWidth);
	Ys2 = y1;
	
	//print the variables
	print("w, h, B, Div, Ys1, Ys2", "("+list[ii]+")");
	print(w, h, B, Div, Ys1, Ys2);
	
	//Loop to measure back, flanks and abdomen		
	for (e = 0; e < 3; e += 1) { //begin of inner loop
	...
	//--------------------------------------------------------------------------------------------------------------------------------
		...
		if (e == 0) { //Cranial body region
			bp = "Cranial";
			makeRectangle(0, 0, w, Ys1); 
			run("Cut");
			newImage(""+bp+"-1", "RGB black", w, Ys1, 1);
			run("Paste");
			newImage(""+bp+"-2", "RGB black", w, h, 1);
			run("Add Image...", "image=["+bp+"-1] x=0 y=0 opacity=100");
		}	
		else if (e == 1) { //Center body region
			bp = "Central";
			makeRectangle(0, Ys1, w, h-Ys1-(h-Ys2)); 
			run("Cut");
			newImage(""+bp+"-1", "RGB black", w, h-Ys1-(h-Ys2), 1);
			run("Paste");
			newImage(""+bp+"-2", "RGB black", w, h, 1);
			run("Add Image...", "image=["+bp+"-1] x=0 y="+Ys1+" opacity=100");
		}
		else { //Caudal body region
			bp = "Caudal";
			makeRectangle(0, Ys2, w, h-Ys2); 
			run("Cut");
			newImage(""+bp+"-1", "RGB black", w, h-Ys2, 1);
			run("Paste");
			newImage(""+bp+"-2", "RGB black", w, h, 1);
			run("Add Image...", "image=["+bp+"-1] x=0 y="+Ys2+" opacity=100");
		}
		...
		//close remaining image windows and clear RAM
		//Head
	...
	selectWindow(image9);
	makeLine(0, Ys1, w, Ys1); //separates cranial from central body region
	run("Flatten");
	makeLine(0, Ys2, w, Ys2); //separates central from caudal body region
	run("Flatten");
...
selectWindow("Log");
saveAs("Text", path+"_CCC_variables_All.txt");
selectWindow("Log");
run("Close");